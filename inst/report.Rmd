...
```{r, include=F}
commit <- strtrim(system("git rev-parse HEAD", intern=TRUE), 7)
datetime <- format.Date(Sys.time())
```

Generated with the [rfaster2](https://github.com/angelovangel/rfaster2) package (commit ` `r commit` `) on `r datetime`.   
...
kableExtra::kbl(
  ...
  # ) %>%
  kable_styling(bootstrap_options = 'hover', full_width = TRUE, fixed_thead = T)
...
df %>%
  ...
  kable_styling(full_width = TRUE, fixed_thead = T)
